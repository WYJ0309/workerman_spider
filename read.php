...
$worker->onWorkerStart = function ($worker){
    echo 'starting.....';
    /**
     * 强烈建议在onWorkerStart回调中初始化数据库连接，避免在Worker::runAll();运行前就初始化连接，在Worker::runAll();
     * 运行前初始化的连接属于主进程，子进程会继承这个连接，主进程和子进程共用相同的数据库连接会导致错误
     */
    global $db;
    $db = new \Workerman\MySQL\Connection('127.0.0.1', '3306', 'root', 'root', 'workman_test');
...
$worker->onMessage = function($connection, $data)
{
    ...
    $connection->send('process is ending...');
};
/**
